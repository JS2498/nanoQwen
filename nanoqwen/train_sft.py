...
@torch.no_grad()
def estimate_val_loss_sft(
    model: Qwen,
    dm: SFTMemmapDataModule,
    batch_size: int,
    block_size: int,
    eval_iters: int,
    device: str,
...
def maybe_load_pretrained(model: Qwen, ckpt_path: str | None, device: str) -> None:
    ...
    # Handle vocab growth between pretraining and SFT (e.g., tokenizer.vocab_size -> len(tokenizer)).
    current_state = model.state_dict()
    resize_keys = ["model.embed_tokens.weight", "lm_head.weight"]
    for k in resize_keys:
        if k not in model_state or k not in current_state:
            continue
        src = model_state[k]
        dst = current_state[k]
        if src.shape == dst.shape:
        ...
        if src.ndim != 2 or dst.ndim != 2 or src.shape[1] != dst.shape[1] or src.shape[0] > dst.shape[0]:
            raise RuntimeError(
                f"Unsupported shape mismatch for {k}: checkpoint={tuple(src.shape)}, model={tuple(dst.shape)}"
            )

        expanded = dst.clone()
        expanded[: src.shape[0]] = src
        model_state[k] = expanded
        print(
            f"resized_pretrained_tensor: {k} {tuple(src.shape)} -> {tuple(dst.shape)} "
            f"(copied first {src.shape[0]} rows)"
        )

    model.load_state_dict(model_state, strict=True)
    print(f"loaded_pretrained: {path}")
...
def train_sft(args: argparse.Namespace) -> None:
    ...
    dm = SFTMemmapDataModule(
        ...
        train_file_name=args.sft_train_file,
        val_file_name=args.sft_val_file,
        train_split=args.train_split,
        seed=args.seed,
        max_seq_len=args.block_size,
        cache_dir=args.sft_cache_dir,
        rebuild_cache=args.rebuild_sft_cache,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sft-train-file", type=str, default="sft_static_train.jsonl")
    parser.add_argument("--sft-val-file", type=str, default=None)
    parser.add_argument("--prompt-field", type=str, default="prompt")
    parser.add_argument("--response-field", type=str, default="response")
    parser.add_argument("--sft-cache-dir", type=str, default="data/cache_sft")
    parser.add_argument("--rebuild-sft-cache", action="store_true")
